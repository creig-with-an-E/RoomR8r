    ...
    if (
      ...
      cards = this.props.searchResults
        ? this.props.searchResults.map(element => (
            <ReviewCard data={element} key={element.id} />
          ))
        : null;
      ...
      <Layout showModalHandle={this.showModalHandle}>
        <section style={styles.sectionStyle} className={classes.sectionStyle}>
          ...
          <h3 className={classes.secondaryHeader}>
            <i className="fa fa-home fa-2x" style={{ color: "#FF5941" }} />{" "}
            WARMING or <i className="fa fa-home fa-2x"></i>{" "}
            <span className={classes.highlightText}>WARNING</span> ?
          </h3>
          <SearchBar
          ...
          <p className={classes.errorStyle}>
            {" "}
            {this.state.addressError ? "Acceptable format is X9X-9X9" : ""}{" "}
          </p>
          <p className={classes.searchHeading}>
            Search for reviews by Postal Code
          </p>
          {spinner}
          {emptyResultsText}
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    findByAddress: (userToken, address) =>
      dispatch(appActions.findReviewByAddress(userToken, address))
  };
};
